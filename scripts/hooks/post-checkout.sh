...
write-stdlog "Post-checkout hook triggered for ref $NEW_HEAD" info

# Optional: custom logic per branch
if [[ "$IS_BRANCH_SWITCH" == "1" ]]; then
  write-stdlog "Branch switch detected from $PREV_HEAD to $NEW_HEAD" success
  # Optional: source extra logic
  # source "$GIT_ROOT/scripts/modules/PostCheckoutActions.sh"
  # run_post_checkout_tasks "$NEW_HEAD"
else
  write-stdlog "File checkout occurred; no branch change." ok
fi
